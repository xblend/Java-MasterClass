...
public class PatternMatching {
    ...
    public static List<Integer> boyerMoore(CharSequence pattern, CharSequence text, CharacterComparator comparator) {
        ...
        if(m>n)
            return matches;
        Map<Character,Integer> lots = buildLastTable(pattern);
        int i=0;
        while(i<=n-m){
            int j = m-1;
            ...
            if(j==-1){
                ...
                int shift = lots.getOrDefault(text.charAt(i+j),-1);
                if(shift<j)
                    i=i+j-shift;
                else
                    i++;
    ...
    public static Map<Character, Integer> buildLastTable(CharSequence pattern) {
        // WRITE YOUR CODE HERE (DO NOT MODIFY METHOD HEADER)!
        Map<Character,Integer> lots = new HashMap<Character,Integer>();
        if(pattern.length()==0)
            return lots;
        for(int i=0;i<pattern.length();i++)
            lots.put(pattern.charAt(i),i);
        return lots;
    }
}